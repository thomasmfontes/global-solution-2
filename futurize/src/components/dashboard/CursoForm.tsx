...
export default function CursoForm({
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            placeholder="Ex: Tecnologia, Negócios, Design..."
            required
          />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="carga-horaria-curso" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Carga Horária (horas) *
          </label>
          <input
            type="number"
            id="carga-horaria-curso"
            min="1"
            value={novoCurso.cargaHoraria || ""}
            onChange={(e) => setNovoCurso({ ...novoCurso, cargaHoraria: parseInt(e.target.value) || 0 })}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            required
          />
        </div>
        <div>
          ...
          <select
            ...
            onChange={(e) => setNovoCurso({ ...novoCurso, nivel: e.target.value as 'Iniciante' | 'Intermediario' | 'Avancado' })}
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          >
      ...
      <div className="flex gap-3">
        ...
        >
          {criando ? (editando ? "Salvando..." : "Criando...") : (editando ? "Salvar Alterações" : "Criar Curso")}
        </button>
        <button
          type="button"
          onClick={onCancel}
